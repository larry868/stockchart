...
func (drawing DrawingCandles) OnRedraw(layer *drawingLayer) {
	...
	for item != nil {
		...
		// x axis: time
		rcandle.O.X = drawArea.O.X + int(xfactor*item.TimeStamp.Sub(drawing.xAxisRange.From).Seconds())
		rcandle.Width = imax(1, int(xfactor*item.Duration.Seconds()))

		// add padding between candles
		xpadding := int(float64(rcandle.Width) * 0.075)
		...
		rwick.O.X = rcandle.Middle().X - rwick.Width/2
